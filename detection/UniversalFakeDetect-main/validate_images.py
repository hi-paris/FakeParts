...
def validate(model, loader, dataset_name, result_folder, save_tensor=False, find_thres=False):
    ...
    if save_tensor:
        tensor_path = os.path.join(result_folder, dataset_name+'_predication_for_pr_roc_curve.pth')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resize', type=int, default=None, help='resize the image')
    ...
    parser.add_argument('--save_tensor', default=False, action='store_true', help='save tensor for PR/ROC curve')
    ...
    with open( os.path.join(opt.result_folder,'acc0.csv'), 'w') as f:
        csv_writer = csv.writer(f, delimiter=',')
        csv_writer.writerow(['dataset', 'real_acc', 'fake_acc', 'tot_accuracy', 'avg_precision'])
    ...
    print("\n ===== dataset_paths ===== ") #AADDDDDDDDDDDD
    print(dataset_paths) #AADDDDDDDDDDDD
    ...
    for dataset_path in tqdm(dataset_paths, total=len(dataset_paths), desc='Number of datasets'):
        ...
        ap, r_acc0, f_acc0, acc0, r_acc1, f_acc1, acc1, best_thres = validate(model, loader, dataset_name, opt.result_folder, save_tensor=opt.save_tensor, find_thres=True)

        with open( os.path.join(opt.result_folder,'acc0.csv'), 'a') as f:
            csv_writer = csv.writer(f, delimiter=',')
            csv_writer.writerow([dataset_name, round(r_acc0, 4), round(f_acc0, 4), round(acc0, 4), round(ap, 4)])

        print(f"Results has been saved for {dataset_name}.")
    
    print(f'\nAll results has been saved to {opt.result_folder}')
