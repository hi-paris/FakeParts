...
class RealFakeVideoDataset(Dataset):
    def __init__(
        ...
        jpeg_quality=None,
        gaussian_sigma=None,
        frame_size=(224, 224)  # enforce resizing
    ):
        assert data_mode in ["wang2020", "ours"]
        self.jpeg_quality = jpeg_quality
        self.gaussian_sigma = gaussian_sigma
        self.transform = transform
        self.resize = transforms.Resize(frame_size) if frame_size is not None else None
        ...
        if isinstance(real_path, str) and isinstance(fake_path, str):
            ...
            for real_p, fake_p in zip(real_path, fake_path):
                r_list, f_list = self.read_path(real_p, fake_p, data_mode, max_sample, shuffle)
                real_list += r_list
                fake_list += f_list
    ...
    def __getitem__(self, idx):
        ...
        try:
            if path.split('.')[-1] in ["mp4", "avi"]:
                frame_generator = sv.get_video_frames_generator(path, stride=1, start=0, end=None)
                ...
                for frame_file in frame_files:
                    frame = Image.open(os.path.join(path, frame_file)).convert("RGB")
                    frames.append(frame)
        except Exception as e:
            print(f"[WARNING] Failed to load frames from {path}: {e}")
            return torch.zeros((1, 3, 224, 224)), label  # fallback dummy tensor
        ...
        shapes = [t.shape for t in transformed_frames]
        if not all(s == shapes[0] for s in shapes):
            print(f"[ERROR] Frame size mismatch in {path}. Shapes: {set(shapes)}")
            return torch.zeros((1, *shapes[0])), label
