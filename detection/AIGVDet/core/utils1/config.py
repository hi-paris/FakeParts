...
class DefaultConfigs(ABC):
    ...
    dataset_root = os.path.join(root_dir, "data")  # ✅ points to data/train/...

    exp_root = os.path.join(dataset_root, "exp")  # stores logs and checkpoints
    _exp_name = ""
    exp_dir = ""
    ckpt_dir = ""
    logs_path = ""
    ...
    def exp_name(self, value: str):
        ...
        self.exp_dir = os.path.join(self.exp_root, self._exp_name)
        self.ckpt_dir = os.path.join(self.exp_dir, "ckpt")
        self.logs_path = os.path.join(self.exp_dir, "logs.txt")
...
def str2bool(v: str) -> bool:
    if isinstance(v, bool):
        return v
    if v.lower() in ("true", "yes", "on", "y", "t", "1"):
        return True
    if v.lower() in ("false", "no", "off", "n", "f", "0"):
        return False
...
# ─── Normalize Datasets as List ───────────────────────────────────────────────────
if isinstance(cfg.datasets, str):
    cfg.datasets = cfg.datasets.split(",")
if isinstance(cfg.datasets_test, str):
    cfg.datasets_test = cfg.datasets_test.split(",")

# Optional debug
# print("DEBUG: dataset_root =", cfg.dataset_root)
