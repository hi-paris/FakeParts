...
def change_codec(input_path, output_path, codec="libx264"):
    start_time = time.time()
    try:
        ffmpeg.input(input_path).output(output_path, vcodec=codec, an=None).run(overwrite_output=True, quiet=True)
        print(f"Codec changed to {codec}")
    except ffmpeg.Error as e:
        print("Error:", e.stderr)
    end_time = time.time()
    inference_time = end_time - start_time
    #print(f"Time for changing codec: {inference_time:.4f} s")
...
def read_mask(validation_mask, fps, n_total_frames, img_size, mask_dilation_iter, frames):
    raw_masks=[]
    # Check if the mask_path is a video file
    if validation_mask.endswith(".mp4"):
        ...
        while True:
            ...
            raw_masks.append(Image.fromarray(frame))
            idx += 1

        cap.release()

    # Check if the mask_path is a directory containing masks frames
    elif os.path.exists(validation_mask):
        mnames = sorted([mp for mp in os.listdir(validation_mask) if mp.endswith(('jpg', 'jpeg', 'png', 'JPG', 'JPEG', 'PNG'))])
        for idx, mp in enumerate(mnames):
            if idx >= n_total_frames:
                break
            
            img = Image.open(os.path.join(validation_mask, mp))

            # If multiple instance in the frame mask, take the mask with id = 1
            if img.mode == 'P':
                id_mask = 1
                img = img.point(lambda i: i == id_mask)
            raw_masks.append(img)

            #raw_masks.append(cv2.imread(os.path.join(validation_mask, mp)))
            # print(mp)
    else:
        raise ValueError("The mask_path is not a valid video file or directory containing mask frames.")
    
    masked_images = []
    masks = []

    for idx, frame in enumerate(raw_masks):
        # Convert the frame to grayscale and then to a binary mask
        mask = frame.convert('L')
        #mask = Image.fromarray(frame[..., ::-1]).convert('L')

        # Resize the mask to the specified image size if necessary
        if mask.size != img_size:
            mask = mask.resize(img_size, Image.NEAREST)
        mask = np.asarray(mask, dtype=np.uint8)

        # Create a binary mask where non-zero values are set to 1
        m = np.array(mask > 0).astype(np.uint8)

        # Apply erosion followed by dilation to the mask to clean up noise
        ...
        # Append the processed mask to the list of masks
        masks.append(mask)

        # Apply the mask to the corresponding frame
        masked_image = np.array(frames[idx]) * (1 - (np.array(mask)[:, :, np.newaxis].astype(np.float32) / 255))

        # Convert the masked image back to an image format and append to the list
...
def read_priori(priori, fps, n_total_frames, img_size):
    ...
    if fps != None and priori_fps != fps:
        cap.release()
        raise ValueError("The frame rate of all input videos needs to be consistent.")
...
def read_video(validation_image, video_length, nframes, max_img_size):
    ...
    elif os.path.exists(validation_image):
        frames = sorted([f for f in os.listdir(validation_image) if f.endswith(('jpg', 'jpeg', 'png', 'JPG', 'JPEG', 'PNG'))])
        vframes = [cv2.cvtColor(cv2.imread(os.path.join(validation_image, f)), cv2.COLOR_BGR2RGB) for f in frames]
        fps = None
        n_total_frames = len(vframes)
        n_clip = int(np.ceil(n_total_frames/nframes))
        frames = vframes
    else:
        raise ValueError("The video_path is not a valid video file or directory containing video frames.")
...
class DiffuEraser:
    ...
    def forward(self, validation_image, validation_mask, priori, output_path,
                max_img_size = 1280, video_length=2, mask_dilation_iter=4, nframes=22,
                save_fps=24, seed=None, revision = None, guidance_scale=None, blended=True):
        validation_prompt = ""  # 
        ...
        default_fps = save_fps if fps is None else fps
        ...
        print("finished")
        ##############################################
        output_path2 = output_path.split(".")[0]+"_codec.mp4"
        change_codec(output_path, output_path2)
