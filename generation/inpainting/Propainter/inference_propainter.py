...
def write_video_metadata(video_file, video_name, output_dir, video_metadata_df = None):

    json_output_file_name = video_name + "metadata.json"
    ...
    # read video metadata
    vid = ffmpeg.probe(video_file)
    metadata = (vid['streams'])[0]


    # add external metadata
    # meta data from the parquet master video file
    DeepFakeVideo = {"DeepFakeVideo" : {
    #"videoid" : int(video_metadata_df["videoid"].iloc[0]),
    #"contentUrl" : video_metadata_df["contentUrl"].iloc[0],
    #"duration" : video_metadata_df["duration"].iloc[0],
    #"page_dir" : video_metadata_df["page_dir"].iloc[0],
    #"description" : video_metadata_df["name"].iloc[0],
    "videoid" : None,
    "contentUrl" : None,
    "duration" : None,
    "page_dir" : None,
    ...
    "method" : "Inpainting with Propainter using Grounded-SAM-2 as a mask generator",
    "model" : "sczhou/ProPainter",
    "start_frame_1" : int (start_frame_1),
    "end_frame_1" : int (end_frame_1)
    }}

    merged_metadata = {**metadata, **DeepFakeVideo}

    print(merged_metadata)
    
    # This returns None
    #combined_dict = Merge(metadata, DeepFakeVideo)

    
    # Serializing json
    json_object = json.dumps(merged_metadata, indent=4)
    
    # Writing to sample.json
    with open(json_output_file_path, "w") as outfile:
        outfile.write(json_object)

def inference_propainter(video, mask, output,
                         resize_ratio=1.0, height=-1, width=-1,
                         mask_dilation=4, ref_stride=10, neighbor_length=10,
                         subvideo_length=80, raft_iter=20, mode='video_inpainting',
                         scale_h=1.0, scale_w=1.2, save_fps=24, save_frames=False,
                         fp16=False, name=''):
    ...
    if name : video_name += "_" + name 
    if mask_dilation != 4 : video_name += "_dilation" + str(mask_dilation)
    save_root = output
    ...
    imageio.mimwrite(os.path.join(save_root, 'masked_in.mp4'),
                     ...
                     fps=fps, 
                     quality=7, 
                     macro_block_size=1) #Forced same size as input size
    '''
    imageio.mimwrite(os.path.join(save_root, video_name + '.mp4'), 
                     comp_frames, 
                     fps=fps, 
                     quality=7, 
                     macro_block_size=1) #Forced same size as input size
    # save metadata
    #write_video_metadata(video, video_name, save_root)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-i', '--video', type=str, default='inputs_test/object_removal/bmx-trees', help='Path of the input video or image folder.')
    parser.add_argument(
        '-m', '--mask', type=str, default='inputs_test/object_removal/bmx-trees_mask', help='Path of the mask(s) or mask folder.')
    parser.add_argument(
        '-o', '--output', type=str, default='results/bmx-trees', help='Output folder. Default: results')
    parser.add_argument(
    ...
    parser.add_argument(
        '--mask_dilation', type=int, default=4, help='Mask dilation for video and flow masking. Default = 4')
    parser.add_argument(
    ...
    parser.add_argument(
        '-n', '--name', type=str, default='', help='Name complement')

    args = parser.parse_args()

    #args_dict = vars(args)
    #inference_propainter(**args_dict)

    inference_propainter(args.video, args.mask, args.output,
                         args.resize_ratio, args.height, args.width,
                         args.mask_dilation, args.ref_stride, args.neighbor_length,
                         args.subvideo_length, args.raft_iter, args.mode,
                         args.scale_h, args.scale_w, args.save_fps, args.save_frames,
                         args.fp16, args.name)
