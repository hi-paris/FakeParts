...
def object_detection_and_segmentation(
    ...
    class_names = np.array(results["labels"])
    class_ids = np.array(list(range(len(class_names))))
    inds = bboxes_area_rank_indices(input_boxes)
    rank_bboxes = input_boxes[inds]

    #option 1 take the biggest box
    if sampling_method == "take_biggest" :
        n = 0
    
    #option 2 : among the n greatest area take at random
    if sampling_method == "radom_among_n" :
        #select a random int between 0 and n
        n = np.random.randint(0, min(number_of_boxes, len(rank_bboxes)))
        print("n : ", n)
        print("number of boxes : ", number_of_boxes)
        print("len(rank_bboxes) : ", len(rank_bboxes))
    
    input_boxes = rank_bboxes[n:n+1]
    class_names = class_names[inds[n:n+1]]
    class_ids = class_ids[inds[n:n+1]]
...
"""
Fonctions for automated Florence-2 + SAM 2 pipelines
"""
def bboxes_area_rank_indices(bboxes):
