...
def glob_function(model_id, frame_directory_path, text_prompt, save_tracking_results_dir, output_video_path, prompt_type_for_video, number, merge):
    ...
    frame_names = [
        p for p in os.listdir(FRAME_DIRECTORY_PATH)
        if os.path.splitext(p)[-1] in [".jpg", ".jpeg", ".JPG", ".JPEG"]
    ]
    frame_names.sort(key=lambda p: int(os.path.splitext(p)[0]))

    # init video predictor state
    inference_state = video_predictor.init_state(video_path=FRAME_DIRECTORY_PATH)
    ...
    img_path = os.path.join(FRAME_DIRECTORY_PATH, frame_names[ann_frame_idx])
    ...
    SAVE_ANNOTATED_FRAMES_DIR = os.path.join(SAVE_TRACKING_RESULTS_DIR, "annotated_frames")
    if not os.path.exists(SAVE_ANNOTATED_FRAMES_DIR):
        os.makedirs(SAVE_ANNOTATED_FRAMES_DIR)
    ...
    # Kernel to blur the mask
    kernel = np.ones((3, 3), np.uint8)

    for frame_idx, segments in video_segments.items():
        img = cv2.imread(os.path.join(FRAME_DIRECTORY_PATH, frame_names[frame_idx]))
        ...
        cv2.imwrite(os.path.join(SAVE_ANNOTATED_FRAMES_DIR, f"annotated_frame_{frame_idx:05d}.jpg"), annotated_frame)
        ...
        if merge == 'True':
            ...
            merged_mask = cv2.dilate(merged_mask, kernel, iterations=3)
            ...
            for obj_id, mask in zip(object_ids, masks):
                ...
                merged_mask = cv2.dilate(mask, kernel, iterations=3)
    ...
    create_video_from_images(SAVE_ANNOTATED_FRAMES_DIR, OUTPUT_VIDEO_PATH)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-f', '--frame_dir', type=str, help='Directory path of the video frames.')
    parser.add_argument(
    ...
    parser.add_argument(
        '-t', '--text_prompt', type=str, default="", help='Text prompt for grounding.')
    parser.add_argument(
    ...
    frame_directory_path = args.frame_dir
    model_id = args.model
...
##Adapt the name of the directory regarding of the arguments

    if not text_prompt:
        text_prompt = os.path.split(frame_directory_path)[1]+"."
    ...
    if number != 1 and number != -1:
        save_tracking_results_dir += "_"+str(number)
...
##Call the global function
    glob_function(model_id, frame_directory_path, text_prompt, save_tracking_results_dir, output_video_path, prompt_type_for_video, number, merge)
